...
void Graph::show()
{
    std::cout<<"show graph data:"<<std::endl;
    Graph::VertexIDMap verts = vertices();
    for (Graph::VertexIDMap::iterator itV = verts.begin(); itV != verts.end(); itV ++){
        Graph::Vertex* v = itV->second;
        int id = v->id();
        std::cout<<"node: "<<id<<", neibor(edge, dist): ";
        for (Graph::EdgeSet::iterator nei = v->edges().begin(); nei !=v->edges().end(); nei++)
        {
            Graph::Edge* e = *nei;
            int neiid = (e->from()->id() == id) ? e->to()->id():e->from()->id();
            std::cout<<neiid<<"("<<e->id()<<","<<e->cost()<<")";
        }
        std::cout<<std::endl;
    }
}
